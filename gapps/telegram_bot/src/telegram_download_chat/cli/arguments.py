...
def parse_args(argv: Optional[list[str]] = None) -> CLIOptions:
    ...
    parser.add_argument(
        "chat", nargs="?", help="Chat identifier (username, phone number, or chat ID). Use 'all' to download all available chats."
    )
    parser.add_argument(
        "--all-chats",
        action="store_true",
        help="Download all available chats and messages (same as using 'all' as chat identifier)",
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    # Handle all-chats option
    if args.all_chats or (args.chat and args.chat.lower() == "all"):
        args.all_chats = True
        args.chat = "all"
        chat_list = ["all"]
