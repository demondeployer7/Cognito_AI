...
async def async_main() -> int:
    ...
    try:
        ...
        # Handle all chats case
        if chats and (chats[0] == "all" or getattr(args, 'all_chats', False)):
            downloader.logger.info("Fetching all available chats...")
            try:
                await downloader.connect(cli=True)
                all_chats = []
                async for dialog in downloader.client.iter_dialogs():
                    chat_id = str(dialog.entity.id)
                    chat_title = getattr(dialog.entity, 'title', getattr(dialog.entity, 'first_name', f'Chat_{chat_id}'))
                    all_chats.append(chat_id)
                    downloader.logger.info(f"Found chat: {chat_title} (ID: {chat_id})")
                chats = all_chats
                downloader.logger.info(f"Found {len(chats)} chats to download")
                if not chats:
                    downloader.logger.warning("No chats found")
                    return 0
            except Exception as e:
                downloader.logger.error(f"Failed to fetch chats: {e}")
                return 1
        elif not chats:
            downloader.logger.error("Chat identifier is required. Use 'all' to download all chats or provide specific chat identifiers.")
            return 1
